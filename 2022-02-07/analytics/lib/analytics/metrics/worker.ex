defmodule Analytics.Metrics.Worker do
  use GenServer, restart: :temporary

  @registry Analytics.Metrics.Registry
  ...
  defp upsert!(path, counter) do
    import Ecto.Query
    date = Date.utc_today()
    query = from(m in Analytics.Metrics.Metric, update: [inc: [counter: ^counter]])

    Analytics.Repo.insert!(
      %Analytics.Metrics.Metric{date: date, path: path, counter: counter},
      on_conflict: query,
